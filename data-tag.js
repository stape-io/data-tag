    ...
    sendPixel = function (url) {
      ...
      if (
        ...
        img.onload = img.onerror = function () {
          ...
          setCookieRunningCount--;
          if (
            (!xhr || xhr.readyState === 4) && // server container must be finished to be sure no more cookies will be received
            dataLayerEventName &&
        ...
        for (
          ...
          a: {
            ...
            var lines = replacedResponse.substring(0, nextSeparationPos).split('\n'),
              linesIterator =
                'undefined' != typeof Symbol && Symbol.iterator && lines[Symbol.iterator];
...
function dataTagGetData(containerId, eventId, useOwnDataModel, useOnlyCurrentEventObj) {
  ...
  var currentEventObj;
  if (typeof eventId !== 'undefined' && (useOwnDataModel || useOnlyCurrentEventObj)) {
    try {
...
function dataTagGetOwnDataModel(dataLayerArray) {
  try {
    ...
    var typesRegex = /\[object (Boolean|Number|String|Function|Array|Date|RegExp)\]/;
    ...
    for (var i = 0; i < dataLayerArray.length; i++) {
      ...
      if (isArguments(item)) {
        ...
        if (command === 'js' && item.length >= 2 && item[1].getTime) {
          ...
          if (item.length === 2 && isPlainObject(item[1])) {
            ...
            if (isPlainObject(valueToSet) || getValueType(valueToSet) == 'array') {
              processedItem[item[1]] = cloneObject(valueToSet);
            } else {
              processedItem[item[1]] = valueToSet;
            }
          }
        } else if (command === 'event' && item.length >= 2 && isString(item[1])) {
          var eventParams = item.length > 2 && isPlainObject(item[2]) ? cloneObject(item[2]) : {};
          processedItem = {
            event: item[1],
      ...
      for (var key in item) {
        if (hasOwnProperty(item, key) && key !== '_clear') {
          if (shouldClear) {
            var clearObject = convertDotNotationToNestedObject(key, undefined);
            recursiveMerge(clearObject, dataModel);
          }
          var newNestedObject = convertDotNotationToNestedObject(key, item[key]);
